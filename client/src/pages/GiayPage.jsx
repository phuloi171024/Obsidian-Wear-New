...
export default function GiayPage() {
  ...
  useEffect(() => {
    const fetchFilteredProducts = async () => {
      try {
        ...
        const filters = { category: "Giày", sort: sortOption };
        ...
        console.error("Lỗi khi tải dữ liệu lọc sản phẩm Giày:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleToggleWishlist = async (e, productId) => {
    ...
    try {
      ...
      if (res.ok && data.status) {
        if (data.is_favorite) setFavoriteIds((prev) => [...prev, productId]); 
  ...
  const openVariantModal = (e, product, buyNow = false) => {
    ...
    const token = localStorage.getItem("access_token");
    if (!token) { toast.error("Vui lòng đăng nhập để mua hàng!"); navigate("/login"); return; }
    setActiveProduct(product); setIsBuyNow(buyNow);
  ...
  const confirmAddToCart = async () => {
    try {
      ...
      const token = localStorage.getItem("access_token");
      if (!token) { toast.error("Phiên đăng nhập đã hết hạn!"); navigate("/login"); return; }
      
      const variants = activeProduct?.variants || [];
      const matchedVariant = variants.find((v) => v.size === selectedSize && v.color === selectedColor);
      if (!matchedVariant) { toast.error("Phiên bản sản phẩm này hiện không có sẵn trong hệ thống!"); return; }

      const res = await fetch("http://localhost:8000/api/cart/add", {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json", Authorization: `Bearer ${token.trim()}` },
        body: JSON.stringify({ product_variant_id: matchedVariant.id, quantity: 1 }),
      });
      const data = await res.json();
      ...
      if (res.status === 401) { localStorage.removeItem("access_token"); toast.error("Phiên đăng nhập đã hết hạn!"); navigate("/login"); return; }
      toast.error(data.message || "Không thể thêm sản phẩm vào giỏ hàng");
    } catch (error) { toast.error("Không thể kết nối đến máy chủ"); } 
      ...
      <div className="product-page">
        ...
        <section className="product-content">
          <div className="top-bar">
            <h2>Bộ Sưu Tập Giày Thể Thao</h2>
          ...
          <div className="breadcrumb">
            <span>Trang chủ</span><span className="arrow"> &gt; </span><span className="active">Giày</span>
          </div>
          ...
          {loading ? (
            ...
            <div className="shop-product-grid">
              {products.length > 0 ? (
                ...
                <p style={{ textAlign: "center", color: "#777", gridColumn: "1 / -1", padding: "40px" }}>Không tìm thấy sản phẩm giày phù hợp.</p>
              )}
            </div>
          )}
        </section>
        ...
        <div className="custom-modal-overlay">
          <div className="custom-modal-box variant-modal">
            ...
            <div className="variant-options">
              {uniqueSizes.length > 0 && (
                <div className="option-group">
                  ...
                  <div className="option-buttons">
                    {uniqueSizes.map((s) => <button key={s} className={selectedSize === s ? "active" : ""} onClick={() => setSelectedSize(s)}>{s}</button>)}
                  </div>
                </div>
              )}
              {uniqueColors.length > 0 && (
                <div className="option-group">
                  <label>Màu sắc:</label>
                  <div className="option-buttons">
                    {uniqueColors.map((c) => <button key={c} className={selectedColor === c ? "active" : ""} onClick={() => setSelectedColor(c)}>{c}</button>)}
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="custom-modal-actions">
              ...
              <button className="btn-confirm" style={{ background: "#4f46e5", opacity: addingToCart ? 0.7 : 1 }} onClick={confirmAddToCart} disabled={addingToCart}>
